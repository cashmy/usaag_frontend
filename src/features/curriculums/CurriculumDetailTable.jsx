...
export default function CurriculumDetail(props) {
    ...
    useEffect(() => {
        ...
        if (location.state !== null && location.state.id !== undefined) {
            console.log("CurriculumDetail-Location-State: ", location.state)
            setThemeInfo({
                'currThemeId': location.state.id,
                'currThemeName': location.state.name.substr(0, 30)
            });
            getCurriculumDtls(location.state.id)
        }
    }, [loadData, location])
                            ...
                            {isLoading ? (
                                ...
                                recordsAfterPagingAndSorting().map(item => (
                                    <TableRow key={item.id}>
                                        ...
                                        <TableCell>{item.headerId ? item.templateHeader.name : ""}</TableCell>
                                        <TableCell
                                            className={classes.multiLineDesc}
